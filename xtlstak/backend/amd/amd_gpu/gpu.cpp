...
#include "xtlstak/picosha2/picosha2.hpp"
#include "xtlstak/params.hpp"
#include "xtlstak/version.hpp"
#include "xtlstak/net/msgstruct.hpp"
...
size_t InitOpenCLGpu(cl_context opencl_ctx, GpuContext* ctx, const char* source_code)
{
	...
	for(int ii = 0; ii < num_algos; ++ii)
	{
		...
		if(xtlstak::params::inst().AMDCache == false || !clBinFile.good())
		{
			if(xtlstak::params::inst().AMDCache)
...
std::vector<GpuContext> getAMDDevices(int index)
{
	...
	for (size_t k = 0; k < num_devices; k++)
	{
		...
		std::string selectedOpenCLVendor = xtlstak::params::inst().openCLVendor;
...
size_t InitOpenCL(GpuContext* ctx, size_t num_gpus, size_t platform_idx)
{
	...
	std::string platformName(platformNameVec.data());
	if(xtlstak::params::inst().openCLVendor == "AMD" && platformName.find("Advanced Micro Devices") == std::string::npos)
	{
	...
	for(int i = 0; i < num_gpus; ++i)
	{
		...
		const std::string backendName = xtlstak::params::inst().openCLVendor;
...
size_t XMRSetJob(GpuContext* ctx, uint8_t* input, size_t input_len, uint64_t target, xtlstak_algo miner_algo)
{
...
size_t XMRRunJob(GpuContext* ctx, cl_uint* HashOutput, xtlstak_algo miner_algo)
{
	// switch to the kernel storage
	int kernel_storage = miner_algo == ::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgo() ? 0 : 1;
