...
#include "xtlstak/misc/utility.hpp"

#ifndef CONF_NO_HTTPD
#	include "xtlstak/http/httpd.hpp"
#endif
...
std::string get_multipool_entry(bool& final)
{
	...
	std::string pool;
	std::cout<<"- Pool address: e.g. " << jconf::GetDefaultPool(xtlstak::params::inst().currency.c_str()) << std::endl;
	std::cin >> pool;
...
int do_benchmark(int block_version, int wait_sec, int work_sec)
{
	...
	std::vector<xtlstak::iBackend*>* pvThreads;
	 ...
	 * \todo fix this issue
	 */
	xtlstak::miner_work benchWork = xtlstak::miner_work("", work, 84, 0, false, 0);
	printer::inst()->print_msg(L0, "Start a %d second benchmark...",work_sec);
	xtlstak::globalStates::inst().switch_work(benchWork, dat);
	...
	for (uint32_t i = 0; i < pvThreads->size(); i++)
	{
		...
		auto bType = static_cast<xtlstak::iBackend::BackendType>(pvThreads->at(i)->backendType);
