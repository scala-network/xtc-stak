...
#	include "xtcstak/http/httpd.hpp"
#endif
...
std::string get_multipool_entry(bool& final)
{
	...
	std::string pool;
	std::cout<<"- Pool address: e.g. " << jconf::GetDefaultPool(xtcstak::params::inst().currency.c_str()) << std::endl;
	std::cin >> pool;
...
*/
	...
	printer::inst()->print_str("-------------------------------------------------------------------\n");
	printer::inst()->print_str("Torque (XTC) <3 Stak \n");       
	printer::inst()->print_str("-------------------------------------------------------------------\n");
...
int do_benchmark(int block_version, int wait_sec, int work_sec)
{
	...
	std::vector<xtcstak::iBackend*>* pvThreads;
	 ...
	 * \todo fix this issue
	 */
	xtcstak::miner_work benchWork = xtcstak::miner_work("", work, 84, 0, false, 0);
	printer::inst()->print_msg(L0, "Start a %d second benchmark...",work_sec);
	xtcstak::globalStates::inst().switch_work(benchWork, dat);
	...
	for (uint32_t i = 0; i < pvThreads->size(); i++)
	{
		...
		auto bType = static_cast<xtcstak::iBackend::BackendType>(pvThreads->at(i)->backendType);
