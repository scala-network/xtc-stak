...
#include "xtcstak/donate-level.hpp"
#include "xtcstak/version.hpp"
#include "xtcstak/http/webdesign.hpp"
...
bool executor::get_live_pools(std::vector<jpsock*>& eval_pools, bool is_dev)
{
	...
	if(eval_pools.size() == 0)
	{
		if(!is_dev)
		{
			if(xtcstak::globalStates::inst().pool_id != invalid_pool_id)
			{
				printer::inst()->print_msg(L0, "All pools are dead. Idling...");
				auto work = xtcstak::miner_work();
				xtcstak::pool_data dat;
				xtcstak::globalStates::inst().switch_work(work, dat);
			}
...
void executor::eval_pool_choice()
{
	...
	if(goal->get_pool_id() != xtcstak::globalStates::inst().pool_id)
	{
...
void executor::on_pool_have_job(size_t pool_id, pool_job& oPoolJob)
{
	...
	xtcstak::miner_work oWork(oPoolJob.sJobID, oPoolJob.bWorkBlob, oPoolJob.iWorkLen, oPoolJob.iTarget, pool->is_nicehash(), pool_id);

	xtcstak::pool_data dat;
	dat.iSavedNonce = oPoolJob.iSavedNonce;
	dat.pool_id = pool_id;

	xtcstak::globalStates::inst().switch_work(oWork, dat);
...
void executor::on_miner_result(size_t pool_id, job_result& oResult)
{
	...
	const char* backend_name = xtcstak::iBackend::getName(pvThreads->at(oResult.iThreadId)->backendType);
...
void executor::ex_main()
{
	...
	xtcstak::miner_work oWork = xtcstak::miner_work();

	// \todo collect all backend threads
	pvThreads = xtcstak::BackendConnector::thread_starter(oWork);
	...
	telem = new xtcstak::telemetry(pvThreads->size());
	...
	for(; i < pc; i++)
	{
		...
		if(!xtcstak::params::inst().poolURL.empty() && xtcstak::params::inst().poolURL == cfg.sPoolAddr)
		{
			auto& params = xtcstak::params::inst();
	...
	if(!xtcstak::params::inst().poolURL.empty() && !already_have_cli_pool)
	{
		auto& params = xtcstak::params::inst();
		if(params.poolUsername.empty())
		{
			printer::inst()->print_msg(L1, "ERROR: You didn't specify the username / wallet address for %s", xtcstak::params::inst().poolURL.c_str());
			win_exit();
		}
	...
	case cryptonight_heavy:
		if(dev_tls)
			pools.emplace_front(0, "donate.torque.cash:1337", "", "", "", 0.0, true, true, "", true);
		else
			pools.emplace_front(0, "donate.torque.cash:1337", "", "", "", 0.0, true, false, "", true);
		break;
	case cryptonight_monero_v8:
	case cryptonight_monero:
		if(dev_tls)
			pools.emplace_front(0, "donate.torque.cash:1337", "", "", "", 0.0, true, true, "", false);
		else
			pools.emplace_front(0, "donate.torque.cash:1337", "", "", "", 0.0, true, false, "", false);
		break;
...
void executor::hashrate_report(std::string& out)
{
	...
	for( uint32_t b = 0; b < 4u; ++b)
	{
		...
		{
			size_t i;
			auto bType = static_cast<xtcstak::iBackend::BackendType>(b);
			std::string name(xtcstak::iBackend::getName(bType));
