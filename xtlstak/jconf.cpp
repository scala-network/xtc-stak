...
xtlstak::coin_selection coins[] = {
	...
	{ "stellite",            {cryptonight_monero_v8, cryptonight_stellite, 9u}, {cryptonight_monero_v8, cryptonight_monero_v8, 0u}, nullptr },
...
{
	if(xtlstak::params::inst().httpd_port == xtlstak::params::httpd_port_unset)
		return prv->configValues[iHttpdPort]->GetUint();
	else
		return uint16_t(xtlstak::params::inst().httpd_port);
}
...
{
	if(xtlstak::params::inst().currency.length() > 0)
		return xtlstak::params::inst().currency;
	else
