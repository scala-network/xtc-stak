...
BOOL SelfElevate(const std::string& my_path, const std::string& params)
{
	...
	printer::inst()->print_msg(L0, "This window has been opened because xtc-stak needed to run as administrator.  It can be safely closed now.");
...
VOID RequestElevation()
{
	...
	if(!xtcstak::params::inst().allowUAC)
	{
	...
	SelfElevate(xtcstak::params::inst().minerArg0, xtcstak::params::inst().minerArgs);
}
