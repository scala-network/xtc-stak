...
#include "xtlstak/net/jpsock.hpp"

#include "telemetry.hpp"
#include "xtlstak/backend/miner_work.hpp"
#include "xtlstak/backend/globalStates.hpp"
#include "xtlstak/backend/backendConnector.hpp"
#include "xtlstak/backend/iBackend.hpp"

#include "xtlstak/jconf.hpp"
#include "xtlstak/misc/console.hpp"
#include "xtlstak/donate-level.hpp"
#include "xtlstak/version.hpp"
#include "xtlstak/http/webdesign.hpp"
...
bool executor::get_live_pools(std::vector<jpsock*>& eval_pools, bool is_dev)
{
	...
	if(eval_pools.size() == 0)
	{
		if(!is_dev)
		{
			if(xtlstak::globalStates::inst().pool_id != invalid_pool_id)
			{
				printer::inst()->print_msg(L0, "All pools are dead. Idling...");
				auto work = xtlstak::miner_work();
				xtlstak::pool_data dat;
				xtlstak::globalStates::inst().switch_work(work, dat);
			}
...
void executor::eval_pool_choice()
{
	...
	if(goal->get_pool_id() != xtlstak::globalStates::inst().pool_id)
	{
...
void executor::on_pool_have_job(size_t pool_id, pool_job& oPoolJob)
{
	...
	xtlstak::miner_work oWork(oPoolJob.sJobID, oPoolJob.bWorkBlob, oPoolJob.iWorkLen, oPoolJob.iTarget, pool->is_nicehash(), pool_id);

	xtlstak::pool_data dat;
	dat.iSavedNonce = oPoolJob.iSavedNonce;
	dat.pool_id = pool_id;

	xtlstak::globalStates::inst().switch_work(oWork, dat);
...
void executor::on_miner_result(size_t pool_id, job_result& oResult)
{
	...
	const char* backend_name = xtlstak::iBackend::getName(pvThreads->at(oResult.iThreadId)->backendType);
...
void executor::ex_main()
{
	...
	xtlstak::miner_work oWork = xtlstak::miner_work();

	// \todo collect all backend threads
	pvThreads = xtlstak::BackendConnector::thread_starter(oWork);
	...
	telem = new xtlstak::telemetry(pvThreads->size());
	...
	for(; i < pc; i++)
	{
		...
		if(!xtlstak::params::inst().poolURL.empty() && xtlstak::params::inst().poolURL == cfg.sPoolAddr)
		{
			auto& params = xtlstak::params::inst();
	...
	if(!xtlstak::params::inst().poolURL.empty() && !already_have_cli_pool)
	{
		auto& params = xtlstak::params::inst();
		if(params.poolUsername.empty())
		{
			printer::inst()->print_msg(L1, "ERROR: You didn't specify the username / wallet address for %s", xtlstak::params::inst().poolURL.c_str());
			win_exit();
		}
	...
	case cryptonight_heavy:
		if(dev_tls)
			pools.emplace_front(0, "donate.stellite.cash:1337", "", "", "", 0.0, true, true, "", true);
		else
			pools.emplace_front(0, "donate.stellite.cash:1337", "", "", "", 0.0, true, false, "", true);
		break;
	case cryptonight_monero_v8:
	case cryptonight_monero:
		if(dev_tls)
			pools.emplace_front(0, "donate.stellite.cash:1337", "", "", "", 0.0, true, true, "", false);
		else
			pools.emplace_front(0, "donate.stellite.cash:1337", "", "", "", 0.0, true, false, "", false);
		break;
...
void executor::hashrate_report(std::string& out)
{
	...
	for( uint32_t b = 0; b < 4u; ++b)
	{
		...
		{
			size_t i;
			auto bType = static_cast<xtlstak::iBackend::BackendType>(b);
			std::string name(xtlstak::iBackend::getName(bType));
