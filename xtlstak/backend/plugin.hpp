...
#ifdef WIN32
		fn_startBackend = (startBackend_t) GetProcAddress(libBackend, "xtlstak_start_backend");
		if (!fn_startBackend)
		{
			std::cerr << "WARNING: backend plugin " << libName << " contains no entry 'xtlstak_start_backend': " <<GetLastError()<< std::endl;
		}
#else
		// reset last error
		dlerror();
		fn_startBackend = (startBackend_t) dlsym(libBackend, "xtlstak_start_backend");
		const char* dlsym_error = dlerror();
		if(dlsym_error)
		{
			std::cerr << "WARNING: backend plugin " << libName << " contains no entry 'xtlstak_start_backend': " << dlsym_error << std::endl;
		}
#endif
	}
