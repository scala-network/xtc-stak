...
#include "xtlstak/misc/console.hpp"
#include "xtlstak/backend/iBackend.hpp"
#include "xtlstak/backend/globalStates.hpp"
#include "xtlstak/misc/configEditor.hpp"
#include "xtlstak/backend/cpu/cpuType.hpp"
#include "xtlstak/params.hpp"
#include "jconf.hpp"

#include "xtlstak/misc/executor.hpp"
#include "minethd.hpp"
#include "xtlstak/jconf.hpp"

#include "hwlocMemory.hpp"
#include "xtlstak/backend/miner_work.hpp"
...
bool minethd::self_test()
{
	...
	for(int algo_idx = 0; algo_idx < 2; ++algo_idx)
	{
		...
		if(algo == cryptonight)
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight);
			hashf("This is a test", 14, out, ctx);
			bResult = bResult &&  memcmp(out, "\xa0\x84\xf0\x1d\x14\x37\xa0\x9c\x69\x85\x40\x1b\x60\xd4\x35\x54\xae\x10\x58\x02\xc5\xf5\xd8\xa9\xb3\x25\x36\x49\xc0\xbe\x66\x05", 32) == 0;

			hashf_multi = func_multi_selector<2>(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight);
			...
			hashf_multi = func_multi_selector<2>(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight);
			hashf_multi("The quick brown fox jumps over the lazy dogThe quick brown fox jumps over the lazy log", 43, out, ctx);
			bResult = bResult &&  memcmp(out, "\x3e\xbb\x7f\x9f\x7d\x27\x3d\x7c\x31\x8d\x86\x94\x77\x55\x0c\xc8\x00\xcf\xb1\x1b\x0c\xad\xb7\xff\xbd\xf6\xf8\x9f\x3a\x47\x1c\x59"
					"\xb4\x77\xd5\x02\xe4\xd8\x48\x7f\x42\xdf\xe3\x8e\xed\x73\x81\x7a\xda\x91\xb7\xe2\x63\xd2\x91\x71\xb6\x5c\x44\x3a\x01\x2a\x41\x22", 64) == 0;

			hashf_multi = func_multi_selector<3>(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight);
			...
			hashf_multi = func_multi_selector<4>(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight);
			...
			hashf_multi = func_multi_selector<5>(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight);
		...
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_lite);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight_lite);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			bResult = bResult &&  memcmp(out, "\x5a\x24\xa0\x29\xde\x1c\x39\x3f\x3d\x52\x7a\x2f\x9b\x39\xdc\x3d\xb3\xbc\x87\x11\x8b\x84\x52\x9b\x9f\x0\x88\x49\x25\x4b\x5\xce", 32) == 0;
		}
		else if(algo == cryptonight_monero)
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_monero);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight_monero);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			bResult = bResult &&  memcmp(out, "\x1\x57\xc5\xee\x18\x8b\xbe\xc8\x97\x52\x85\xa3\x6\x4e\xe9\x20\x65\x21\x76\x72\xfd\x69\xa1\xae\xbd\x7\x66\xc7\xb5\x6e\xe0\xbd", 32) == 0;
		}
		else if(algo == cryptonight_monero_v8)
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_monero_v8);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight_monero_v8);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			bResult &= memcmp(out, "\x35\x3f\xdc\x06\x8f\xd4\x7b\x03\xc0\x4b\x94\x31\xe0\x05\xe0\x0b\x68\xc2\x16\x8a\x3c\xc7\x33\x5c\x8b\x9b\x30\x81\x56\x59\x1a\x4f", 32) == 0;
		}
		else if(algo == cryptonight_aeon)
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_aeon);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight_aeon);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			bResult = bResult &&  memcmp(out, "\xfc\xa1\x7d\x44\x37\x70\x9b\x4a\x3b\xd7\x1e\xf3\xed\x21\xb4\x17\xca\x93\xdc\x86\x79\xce\x81\xdf\xd3\xcb\xdd\xa\x22\xd7\x58\xba", 32) == 0;
		}
		else if(algo == cryptonight_ipbc)
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_ipbc);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight_ipbc);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
		...
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_stellite);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight_stellite);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			bResult = bResult &&  memcmp(out, "\xb9\x9d\x6c\xee\x50\x3c\x6f\xa6\x3f\x30\x69\x24\x4a\x0\x9f\xe4\xd4\x69\x3f\x68\x92\xa4\x5c\xc2\x51\xae\x46\x87\x7c\x6b\x98\xae", 32) == 0;
		}
		else if(algo == cryptonight_masari)
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_masari);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight_masari);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			bResult = bResult &&  memcmp(out, "\xbf\x5f\xd\xf3\x5a\x65\x7c\x89\xb0\x41\xcf\xf0\xd\x46\x6a\xb6\x30\xf9\x77\x7f\xd9\xc6\x3\xd7\x3b\xd8\xf1\xb5\x4b\x49\xed\x28", 32) == 0;
		}
		else if(algo == cryptonight_heavy)
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_heavy);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight_heavy);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			bResult = bResult &&  memcmp(out, "\xf9\x44\x97\xce\xb4\xf0\xd9\x84\xb\x9b\xfc\x45\x94\x74\x55\x25\xcf\x26\x83\x16\x4f\xc\xf8\x2d\xf5\xf\x25\xff\x45\x28\x2e\x85", 32) == 0;
		}
		else if(algo == cryptonight_haven)
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_haven);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), true, xtlstak_algo::cryptonight_haven);
			hashf("This is a test This is a test This is a test", 44, out, ctx);
			bResult = bResult &&  memcmp(out, "\xc7\xd4\x52\x9\x2b\x48\xa5\xaf\xae\x11\xaf\x40\x9a\x87\xe5\x88\xf0\x29\x35\xa3\x68\xd\xe3\x6b\xce\x43\xf6\xc8\xdf\xd3\xe3\x9", 32) == 0;
		}
		else if(algo == cryptonight_bittube2)
		{
			...
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_bittube2);
		...
		{
			hashf = func_selector(::jconf::inst()->HaveHardwareAes(), false, xtlstak_algo::cryptonight_superfast);
...
template<size_t N>
minethd::cn_hash_fun minethd::func_multi_selector(bool bHaveAes, bool bNoPrefetch, xtlstak_algo algo, const std::string& asm_version_str)
{
...
minethd::cn_hash_fun minethd::func_selector(bool bHaveAes, bool bNoPrefetch, xtlstak_algo algo)
{
