...
std::vector<iBackend*>* BackendConnector::thread_starter(miner_work& pWork)
{
	...
	if(params::inst().useAMD)
	{
		...
		plugin amdplugin;
		amdplugin.load(backendName, "xtlstak_opencl_backend");
	...
	{
		plugin nvidiaplugin;
		std::vector<std::string> libNames = {"xtlstak_cuda_backend_cuda10_0", "xtlstak_cuda_backend_cuda9_2", "xtlstak_cuda_backend"};
		size_t numWorkers = 0u;
