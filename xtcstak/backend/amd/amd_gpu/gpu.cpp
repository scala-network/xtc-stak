...
#include "xtcstak/picosha2/picosha2.hpp"
...
size_t InitOpenCLGpu(cl_context opencl_ctx, GpuContext* ctx, const char* source_code)
{
	...
	for(int ii = 0; ii < num_algos; ++ii)
	{
		...
		if(xtcstak::params::inst().AMDCache == false || !clBinFile.good())
		{
			if(xtcstak::params::inst().AMDCache)
...
std::vector<GpuContext> getAMDDevices(int index)
{
	...
	for (size_t k = 0; k < num_devices; k++)
	{
		...
		std::string selectedOpenCLVendor = xtcstak::params::inst().openCLVendor;
...
size_t InitOpenCL(GpuContext* ctx, size_t num_gpus, size_t platform_idx)
{
	...
	std::string platformName(platformNameVec.data());
	if(xtcstak::params::inst().openCLVendor == "AMD" && platformName.find("Advanced Micro Devices") == std::string::npos)
	{
	...
	for(int i = 0; i < num_gpus; ++i)
	{
		...
		const std::string backendName = xtcstak::params::inst().openCLVendor;
...
size_t XMRSetJob(GpuContext* ctx, uint8_t* input, size_t input_len, uint64_t target, xtcstak_algo miner_algo)
{
	...
	if(miner_algo == cryptonight_monero || miner_algo == cryptonight_aeon || miner_algo == cryptonight_ipbc || miner_algo == cryptonight_torque || miner_algo == cryptonight_masari || miner_algo == cryptonight_bittube2)
	{
		// Input
...
size_t XMRRunJob(GpuContext* ctx, cl_uint* HashOutput, xtcstak_algo miner_algo)
{
	// switch to the kernel storage
	int kernel_storage = miner_algo == ::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgo() ? 0 : 1;
